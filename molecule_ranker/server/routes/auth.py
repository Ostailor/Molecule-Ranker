...
def oidc_login(request: Request) -> dict[str, Any]:
    ...
    raise HTTPException(status_code=501, detail="OIDC login is not implemented in V0.8.")
...
def oidc_callback(request: Request) -> dict[str, Any]:
    ...
    if not config.enabled:
        raise HTTPException(status_code=404, detail="OIDC is not configured.")
    raise HTTPException(status_code=501, detail="OIDC callback is not implemented in V0.8.")
