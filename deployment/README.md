...
```bash
docker build -f deployment/Dockerfile -t molecule-ranker:2.6.0 .
```
...
1. Mirror `molecule-ranker:2.6.0` into the internal registry or load it with
   `docker load`.
...
V2.0 readiness should confirm version `2.6.0`, `/api/v2`, V2 artifact
