...
if [ -z "${MOLECULE_RANKER_AUTH_SECRET:-}" ] && [ -n "${MOLECULE_RANKER_AUTH_SECRET_FILE:-}" ]; then
  MOLECULE_RANKER_AUTH_SECRET="$(cat "$MOLECULE_RANKER_AUTH_SECRET_FILE")"
  export MOLECULE_RANKER_AUTH_SECRET
fi

if [ -z "${MOLECULE_RANKER_DATABASE_URL:-}" ] && [ -n "${POSTGRES_PASSWORD_FILE:-}" ]; then
  POSTGRES_PASSWORD="$(cat "$POSTGRES_PASSWORD_FILE")"
  POSTGRES_USER="${POSTGRES_USER:-molecule_ranker}"
  POSTGRES_DB="${POSTGRES_DB:-molecule_ranker}"
  POSTGRES_HOST="${POSTGRES_HOST:-postgres}"
  POSTGRES_PORT="${POSTGRES_PORT:-5432}"
  MOLECULE_RANKER_DATABASE_URL="postgresql+psycopg://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${POSTGRES_DB}"
  export MOLECULE_RANKER_DATABASE_URL
fi
