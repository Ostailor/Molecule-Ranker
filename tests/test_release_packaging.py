...
def test_release_manifest_contains_v1_packaging_fields() -> None:
    ...
    assert manifest["version"] == __version__ == "1.4.0"
...
def test_release_notes_render_v1_stabilization_scope() -> None:
    ...
    assert "# molecule-ranker 1.4.0 Release Notes" in notes
...
def test_release_packaging_cli_writes_manifest_and_notes(tmp_path: Path) -> None:
    ...
    assert json.loads(manifest_path.read_text())["version"] == "1.4.0"
    assert "molecule-ranker 1.4.0 Release Notes" in notes_path.read_text()
