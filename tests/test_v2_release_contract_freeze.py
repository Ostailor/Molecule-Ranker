...
def test_v2_release_contracts_freeze_required_surfaces() -> None:
    ...
    assert V2_SCHEMA_VERSION == "2.0"
    assert V2_CONTRACT_VERSION == "v2.0.0"
    assert {
        "api_routes",
        "artifact_schemas",
        "cli_command_groups",
...
def test_v2_artifact_payload_requires_schema_and_contract_versions() -> None:
    ...
    assert "schema_version must be 2.0" in invalid.errors
    assert "contract_version must be v2.0.0" in invalid.errors
...
def test_v2_api_contract_export_cli_writes_v2_schema(tmp_path: Path) -> None:
    ...
    assert payload["info"]["version"] == "2.6.0"
