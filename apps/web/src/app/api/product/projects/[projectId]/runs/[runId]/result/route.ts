...
export async function GET(_request: Request, { params }: ResultRouteContext) {
  try {
    ...
    const data = artifacts.find((artifact) => artifact.artifact_type === "result_bundle_json");

    if (!data) {
      throw productApiError("NOT_FOUND");
    }

    return success({ artifact: data });
  } catch (error) {
